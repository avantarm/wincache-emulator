...
namespace Avantarm\WincacheEmulator;
...
class WincacheEmulator
{
    ...
    public static function cas($key, $old_value, $new_value)
    {
        ...
        if (!\is_int($old_value) || !\is_int($new_value) || static::get($key, $success) !== $old_value || !$success) {
            return false;
        }
            ...
            if ($key !== null) {
                if (\array_key_exists($key, static::$data)) {
                    $info['ucache_entries'][] = [
                        ...
                        'is_session'  => 0,
                        'ttl_seconds' => isset(static::$ttl[$key]) ? (static::$ttl[$key] - time()) : 0,
                        'age_seconds' => 0,
                        'hitcount'    => 0,
                    ];
                }
                ...
                foreach (static::$data as $k => $value) {
                    $info['ucache_entries'][] = [
                        ...
                        'is_session'  => 0,
                        'ttl_seconds' => isset(static::$ttl[$k]) ? (static::$ttl[$k] - time()) : 0,
                        'age_seconds' => 0,
                        'hitcount'    => 0,
                    ];
                }
